...
(async () => {
  ...
  await gate('C2', 'exactly one Relicforge card on the whole shelf', () => {
    ...
    assert(hits.length === 1, `browse-all grid rendered ${hits.length} Relicforge cards`);
    const everywhere = [...shelf.all, ...shelf.picks, ...shelf.sports, ...shelf.themed, ...shelf.classRail]
      .filter(h => h === HREF);
    assert(everywhere.length === 1,
      `Relicforge appears ${everywhere.length} times across all rails — expected exactly one`);
  ...
  await gate('C4', 'not on the Sports rail, and the rail is undisturbed', () => {
    assert(!shelf.sports.includes(HREF), 'Relicforge rendered on the Sports rail');
    ...
    return `Sports rail still ${shelf.sports.length} cards, Relicforge absent from it`;
  });
