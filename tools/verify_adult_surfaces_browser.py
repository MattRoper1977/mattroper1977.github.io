...
PROBE = """(() => {
  ...
  return {
    ...
    anyMailingHref: q('a[href="/mailing-list/"], a[href$="/mailing-list/"]').length,
    anyCommerceHref: q('a[href*="ko-fi.com"], a[href*="paypal"], a[href*="patreon"], a[href*="buymeacoffee"]').length
  };
})()"""

# The pupil sweep, and why it is one assertion rather than four.
#
# Three defects in this estate were the same defect wearing different clothes:
# the chooser's injected account anchor that its static gate could not see, the
# arcade's hard-coded Ko-fi link that no list had ever named, and the arcade's
# hard-coded Members link that no default could reach. Each was found
# individually, and each time the fix named the instance.
#
# So this names none of them. It asks one question of the rendered DOM on every
# surface a child reaches - is there ANY route here to an account, to member
# data, to a mailing list or to a payment page - and it does not care whether
# the route was injected by script, generated by a renderer or typed into the
# markup by hand, because a child cannot tell those apart either.
AFFORDANCE_CLASSES = [
    ("anyAccountHref", "an account route"),
    ("anyMembersHref", "a member-data route"),
    ("anyMailingHref", "a mailing-list route"),
    ("anyCommerceHref", "a commerce route"),
]
...
def measure(overrides: dict[str, bytes] | None = None) -> tuple[list[str], int]:
    ...
    try:
        with sync_playwright() as pw:
            ...
            # The pupil sweep. Every surface a child reaches, every affordance
            # class, in the DOM, regardless of how the route got there.
            pupil = [
                str(e["page"])
                for e in json.loads((ROOT / RECORD).read_text(encoding="utf-8"))["pupilReachableSurfaces"]
            ]
            for rel in pupil:
                try:
                    page.goto(base.rstrip("/") + route_of(rel), wait_until="networkidle", timeout=25000)
                    ...
                    ok(False, f"pupil sweep: {rel} could not be measured ({str(err)[:70]})")
                    continue
                for key, label in AFFORDANCE_CLASSES:
                    ok(dom[key] == 0,
                       f"pupil sweep: {rel} exposes {dom[key]} x {label} in the rendered DOM. "
                       "This surface is declared pupil-reachable in data/adult-surfaces.json; "
                       "it does not matter whether the route was injected, generated or "
                       "hand-written, because a child cannot tell those apart.")
        ...
        if os.environ.get("MBM_BASE_URL", "").strip():
            ...
            if reverted == source:
                ...
                if any("/account/ route(s) in the rendered" in f for f in broke):
                    ...
                    problems += 1
            # The pupil sweep's own control, and the point of it: a HAND-WRITTEN
            # anchor grafted onto a page that currently passes. The fail-closed
            # default cannot touch this one - nothing injected it - so if the
            # sweep catches it, the sweep is measuring routes rather than
            # mechanisms, which is the whole reason it exists.
            pupil_page = "for/pupils/index.html"
            src = (ROOT / pupil_page).read_text(encoding="utf-8")
            grafted = src.replace(
                "</footer>",
                '<a href="/account/">Sign in</a><a href="https://ko-fi.com/madebymattuk">Coffee</a></footer>',
                1,
            )
            if grafted == src:
                print("[FAIL] pupil-sweep control fixture could not be created")
                problems += 1
            else:
                swept, _ = measure({pupil_page: grafted.encode("utf-8")})
                caught_acct = any("exposes 1 x an account route" in f and pupil_page in f for f in swept)
                caught_shop = any("exposes 1 x a commerce route" in f and pupil_page in f for f in swept)
                if caught_acct and caught_shop:
                    print("[PASS] positive control: a hand-written account link AND a commerce link "
                          "grafted onto the pupil homepage are both caught by the sweep")
                else:
                    print(f"[FAIL] pupil-sweep control not detected (account={caught_acct}, "
                          f"commerce={caught_shop})")
                    problems += 1
