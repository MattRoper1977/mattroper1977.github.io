...
module.exports=async function sizingControls(page,valid,check){
  ...
  try{
    ...
    try{
      ...
      file={name:download.suggestedFilename(),length:text.length,boxEmpty:(await box.inputValue())==='',localStorageMatches:await page.evaluate(({text,valid})=>JSON.stringify(window.MBMCartridge.decode(text).localStorage)===JSON.stringify(window.MBMCartridge.decode(valid).localStorage),{text,valid})};
