/* AS1 build-time census, derived from HC3's analyser. Local/session storage are distinct.
 * Parses call sites; never executes page code or reads pupil data. */
'use strict';
const acorn = require('acorn');
const assert = require('node:assert/strict');
const UNKNOWN = Symbol('unresolved'), STORAGE = Symbol('localStorage'), SESSION = Symbol('sessionStorage');
function analyse(scripts) {
  ...
  for(const a of assignments){let n=a.node;while(n?.type==='MemberExpression')n=n.object;if(n?.type==='Identifier'){const b=lookup(a.scope,n.name);if(b)markMutable(b);else if(['localStorage','sessionStorage','window','self','globalThis'].includes(n.name)){const p=a.node.type==='MemberExpression'?a.node.property:null;const field=p&&(a.node.computed?p.value:p.name);if(a.node===n||['localStorage','sessionStorage'].includes(n.name)||['localStorage','sessionStorage'].includes(field))bind(global,n.name,null,true);}}}
  ...
  function value(node, context=new Map(), seen=new Set(), depth=0) {
    ...
    if(node.type==='Identifier'){
      ...
      if(node.name==='localStorage')return STORAGE;if(node.name==='sessionStorage')return SESSION;
    ...
    if(node.type==='MemberExpression'){
      ...
      if(obj?.global&&name==='localStorage')return STORAGE;if(obj?.global&&name==='sessionStorage')return SESSION;
  ...
  function record(node,source,context,via) {
    ...
    if(method==='clear'&&receiver!==STORAGE&&receiver!==SESSION)return;
    if(!['getItem','setItem','removeItem','clear'].includes(method)){if((receiver===STORAGE||receiver===SESSION)&&method===UNKNOWN)results.push({source,line:node.loc.start.line,operation:null,key:null,reason:'storage method unresolved'});return;}
    results.push({source,line:node.loc.start.line,operation:method,storage:receiver===STORAGE?'localStorage':receiver===SESSION?'sessionStorage':null,expression:node.arguments[0]?(scripts.find(s=>s.source===source)?.text.slice(node.arguments[0].start,node.arguments[0].end)||''):'',key:method==='clear'?null:(receiver===STORAGE||receiver===SESSION)&&typeof k==='string'?k:null,reason:!(receiver===STORAGE||receiver===SESSION)?'storage receiver unresolved':method==='clear'?'whole storage clear; not a key':typeof k!=='string'?'key expression unresolved':null,...(via?{via}: {})});
  }
...
if(require.main===module){if(process.argv.includes('--self-test'))selfTest();else{const readline=require('node:readline');const input=readline.createInterface({input:process.stdin});input.on('line',line=>{const row=JSON.parse(line);console.log(JSON.stringify({...row,scripts:undefined,...analyse(row.scripts)}));});}}
