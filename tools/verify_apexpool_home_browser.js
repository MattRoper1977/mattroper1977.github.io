...
(async()=>{
 ...
 const nojsPage=await nojs.newPage();await nojsPage.goto(BASE+'/',{waitUntil:'domcontentloaded'});
 ...
 for(const vp of [{name:'360',width:360,height:900,cols:2,rows:3},{name:'768',width:768,height:1000,cols:3,rows:2},{name:'1200',width:1200,height:1000,cols:3,rows:2}]){
  ...
  await page.goto(BASE+'/',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>document.documentElement.getAttribute('data-doors')==='12',{timeout:10000});
 ...
 page.on('console',m=>{if(m.type()==='error')consoleErrors.push('theme: '+m.text())});page.on('pageerror',e=>pageErrors.push('theme: '+e.message));
 ...
 const calm=await browser.newContext({viewport:{width:1024,height:768},reducedMotion:'reduce'}),calmPage=await calm.newPage();calmPage.on('pageerror',e=>pageErrors.push('reduced: '+e.message));await calmPage.goto(BASE+'/',{waitUntil:'domcontentloaded'});await calmPage.waitForFunction(()=>document.documentElement.getAttribute('data-doors')==='12',{timeout:10000});const reduced=await calmPage.evaluate(()=>({release:!!document.querySelector('[data-release="Apex Pool"]'),sports:document.querySelectorAll('#homeSports [data-sport-game]').length,transition:getComputedStyle(document.querySelector('[data-sport-game="Apex Pool"]')).transitionDuration}));rec('C9-reduced-motion',reduced.release&&reduced.sports===2&&(reduced.transition==='0s'||parseFloat(reduced.transition)<.01),JSON.stringify(reduced));await calm.close();
 rec('C10-zero-console-errors',consoleErrors.length===0,consoleErrors.join(' | '));rec('C10-zero-page-errors',pageErrors.length===0,pageErrors.join(' | '));
