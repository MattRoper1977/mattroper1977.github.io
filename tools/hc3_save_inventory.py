...
def main():
    ...
    if a.self_test:subprocess.run(['node',str(ROOT/'tools/hc3-inventory/analyse.cjs'),'--self-test'],check=True);built_control();return
    ...
    for row in census:
        ...
        routes[route]={'source':source,'sourceSha256':hashlib.sha256(f.read_bytes()).hexdigest(),'publishedSha256':hashlib.sha256(target.read_bytes()).hexdigest(),
    ...
    result={'schema':1,'scope':'Canonical publication rows and migration candidates. Static evidence only; not a transfer permission registry.',
        'sourceHeads':{name:subprocess.check_output(['git','-C',str(root),'rev-parse','HEAD'],text=True).strip() for name,root in [('Site',ROOT),('Lessons',a.lessons)]},
