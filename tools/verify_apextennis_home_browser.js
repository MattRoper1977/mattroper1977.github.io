...
// Sports is additive: protect established cards and derive the rendered count.
...
// New Release is an additive stack. Do not pin the tenant list. The expected
// population is the committed markup; this browser gate proves every box renders
// once with its own derived destination. The static gate independently proves
// those destinations are same-site and resolve in-repo, while AGX-1 proves live
// occupants name real shelf games.
...
if(!RELEASES.length)derivationProblems.push('New Release stack has no occupants');
...
const external=RELEASES.filter(r=>!/^\/[a-z0-9-]+\/$/i.test(RELEASE_HREFS[r]||''));
if(external.length)derivationProblems.push(`New Release main link is not a same-site game route: ${external.join(', ')}`);
const missingTargets=RELEASES.filter(r=>{const h=RELEASE_HREFS[r]||'';return h&&!fs.existsSync(path.join(ROOT,h.replace(/^\//,'') ,'index.html'));});
if(missingTargets.length)derivationProblems.push(`New Release main link has no in-repo target: ${missingTargets.join(', ')}`);
...
function releaseContract(rendered){
 if(rendered.releaseBoxes!==RELEASES.length)return`rendered ${rendered.releaseBoxes} boxes; markup declares ${RELEASES.length}`;
 if(JSON.stringify(rendered.releaseOccupants||rendered.occupants)!==JSON.stringify(RELEASES))return`occupants differ: ${JSON.stringify(rendered.releaseOccupants||rendered.occupants)} vs ${JSON.stringify(RELEASES)}`;
 for(const r of RELEASES){
   const box=rendered.perBox&&rendered.perBox[r];
   if(box&&(!box.title.includes(r)||!box.links.includes(RELEASE_HREFS[r])))return`${r} box does not carry its title/link contract`;
 }
 return null;
}
...
(async()=>{let browser;try{
  ...
  rec('js-off-new-release-contract',releaseContract(off)===null,releaseContract(off)||`${off.releaseBoxes} boxes: ${JSON.stringify(off.occupants)}`);
  ...
  for(const vp of [{name:'360',width:360,height:900,cols:2,rows:4},{name:'768',width:768,height:1000,cols:3,rows:3},{name:'1200',width:1200,height:1000,cols:3,rows:3}]){
    ...
    rec(vp.name+'-new-release-contract',releaseContract(live)===null,releaseContract(live)||`${live.releaseBoxes} box(es): ${JSON.stringify(live.releaseOccupants)}`);
