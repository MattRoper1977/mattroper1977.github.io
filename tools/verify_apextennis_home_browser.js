...
async function waitForDoors(page){
  ...
  await page.waitForFunction(()=>['Apex Golf','Apex Tennis'].every(t=>{
...
(async()=>{let browser;try{
  ...
  const off=await nojsPage.evaluate(()=>{const cards=[...document.querySelectorAll('#homeSports [data-sport-game]')],release=document.querySelector('#newrelease [data-release="Apex Pool"]');return{cards:cards.map(a=>({title:a.dataset.sportGame,href:a.getAttribute('href'),tag:a.tagName})),release:!!release,releaseTitle:release?.querySelector('h3')?.textContent.trim()||'',releaseLinks:release?[...release.querySelectorAll('a')].map(a=>a.getAttribute('href')):[],scrollW:document.documentElement.scrollWidth,innerW:innerWidth};});
  ...
  for(const vp of [{name:'360',width:360,height:900,cols:2,rows:4},{name:'768',width:768,height:1000,cols:3,rows:3},{name:'1200',width:1200,height:1000,cols:3,rows:3}]){
    ...
    rec(vp.name+'-fourteen-rendered-doors-art',live.doorCount==='14'&&live.artCount==='14'&&live.doors.length===14&&live.doors.every(x=>x.art),`${live.doorCount}/${live.artCount}; ${live.doors.length}`);
    ...
    rec(vp.name+'-golf-relative-door',named('Apex Golf').length===1&&named('Apex Golf')[0].href==='apexgolf/'&&named('Apex Golf')[0].art&&named('Apex Golf')[0].loaded,JSON.stringify(named('Apex Golf')));
    rec(vp.name+'-tennis-relative-door',named('Apex Tennis').length===1&&named('Apex Tennis')[0].href==='apextennis/'&&named('Apex Tennis')[0].art&&named('Apex Tennis')[0].loaded,JSON.stringify(named('Apex Tennis')));
  ...
  for(const theme of ['cream','pink','blue','light','dark']){await themePage.goto(BASE+'/',{waitUntil:'domcontentloaded'});await themePage.evaluate(t=>localStorage.setItem('mbm_reading_theme',t),theme);await themePage.reload({waitUntil:'domcontentloaded'});await waitForDoors(themePage);const t=await themePage.evaluate(theme=>({theme,body:document.body.getAttribute('data-theme')||'cream',sports:document.querySelectorAll('#homeSports [data-sport-game]').length,release:document.querySelectorAll('#newrelease [data-release="Apex Pool"]').length,overflow:document.documentElement.scrollWidth===innerWidth}),theme);rec('theme-'+theme,t.body===theme&&t.sports===4&&t.release===1&&t.overflow,JSON.stringify(t));}
