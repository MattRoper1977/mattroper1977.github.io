...
# The generated inline exit region. A game carrying it gives the player a way
# out without /hud.js, which is what the coverage question is actually about.
EXIT_REGION = re.compile(
    r'<!-- MBM-INLINE-EXIT:BEGIN generated by tools/render_inline_exit\.py')
...
def classify(route: str, has_tag: bool, excluded: dict[str, dict],
             has_exit: bool = False) -> str | None:
    """The one rule, in one place so a control can run the same code the gate does.

    THREE states now, not two, and the third was added because the second was
    hiding a real defect behind a true statement. /emberwild/ was reported for
    carrying neither the script nor a declaration - correct, and useless: it
    carries the generated inline exit region, the same one /relicforge/ has.
    What a player needs is not a particular chip, it is a way out.

    Measured before this was widened, because "it has the region" is not the
    same as "the player can leave": with the region present, Emberwild reached
    ZERO focusable elements in 60 Tab presses. Its own key handler bound Tab to
    the game menu and preventDefaulted it, so the exit could never be focused;
    Enter was swallowed the same way, so once reachable it still would not
    fire. Both were repaired in the game, and the exit is now reached in 13
    presses and navigates to /games/, against /relicforge/'s 8. The browser
    half below re-measures that on every run - this static half only decides
    which question to ask of each route.
    """
    ...
    if has_exit:
        return None
    return (f"{route} carries no HUD script, no inline exit region, and is not "
            f"declared in data/hud-coverage.json")
...
def run(base: str, findings: Findings, notes: list[str]) -> None:
    ...
    def _page_src(route: str) -> str:
        return (ROOT / route.strip("/") / "index.html").read_text(encoding="utf-8", errors="replace")

    # Split by HOW a route gives the player a way out, because asking a game
    # with no /hud.js whether its hud chip renders produces true reds about a
    # chip that is correctly absent — 13 of them, all one game, which is how a
    # reader learns to stop reading this job.
    all_routes = [r for r in root_game_routes() if r not in excluded]
    exit_routes = [r for r in all_routes
                   if not HUD_TAG.search(_page_src(r)) and EXIT_REGION.search(_page_src(r))]
    routes = [r for r in all_routes if r not in exit_routes]
    ...
    for route in root_game_routes():
        ...
        if not page_file.is_file():
            ...
            continue
        page_src = page_file.read_text(encoding="utf-8", errors="replace")
        tags = HUD_TAG.findall(page_src)
        has_exit = bool(EXIT_REGION.search(page_src))
        problem = classify(route, bool(tags), excluded, has_exit)
        ...
        elif has_exit and not tags:
            findings.check(True, f"{route}: covered by the inline exit region rather than /hud.js",
                           "the region the browser half exercises below")
        else:
    ...
    with sync_playwright() as pw:
        ...
        for width, height, vp in VIEWPORTS:
            ...
            # Games covered by the inline exit region are held to the same
            # REQUIREMENT by a different control: a way out that renders, is
            # hit-testable, and actually navigates. The keyboard leg is the one
            # that matters and the one that was failing silently — the region
            # was present on /emberwild/ the whole time while Tab reached
            # nothing, so "the region exists" is deliberately not the check.
            for route in exit_routes:
                ...
                try:
                    ...
                    findings.check(False, f"{vp}{route}: the game loads", str(exc)[:110])
                    continue
                findings.check(page.locator("#mbmexit-back").count() == 1,
                               f"{vp}{route}: the inline exit control renders")
                state = page.evaluate(ON_TOP_JS, "mbmexit-back")
                findings.check(state == "ON TOP",
                               f"{vp}{route}: the inline exit control is hit-testable", state)
                box = page.evaluate(
                    "() => { const e = document.getElementById('mbmexit-back');"
                    " if (!e) return null; const r = e.getBoundingClientRect();"
                    " return {w: r.width, h: r.height}; }")
                findings.check(bool(box) and box["w"] >= MIN_CONTROL and box["h"] >= MIN_CONTROL,
                               f"{vp}{route}: the inline exit control has a real hit area",
                               f"{box['w']:.0f}x{box['h']:.0f}" if box else "absent")
                findings.check(not errors, f"{vp}{route}: the game reports no page errors",
                               " | ".join(errors[:2]))
                if vp == "390" or vp == VIEWPORTS[0][2]:
                    presses = 0
                    reached = False
                    for presses in range(1, 61):
                        page.keyboard.press("Tab")
                        hit = page.evaluate(
                            "() => document.activeElement && document.activeElement.id === 'mbmexit-back'")
                        if hit:
                            reached = True
                            break
                    findings.check(reached,
                                   f"{vp}{route}: a keyboard player can REACH the way out",
                                   f"{presses} Tab press(es)" if reached else "not reached in 60 presses")
                    if reached:
                        before = page.url
                        page.keyboard.press("Enter")
                        page.wait_for_timeout(1200)
                        findings.check(page.url != before,
                                       f"{vp}{route}: and activating it navigates away",
                                       f"{before} -> {page.url}")
...
def self_test() -> int:
    ...
    undeclared = [route for route in inventory
                  ...
                  and not HUD_TAG.search(_src(route))
                  and not EXIT_REGION.search(_src(route))]
    if undeclared:
    ...
    # The third state must be able to fail, or widening the rule widened the
    # hole. Strip the region from a copy of the game that relies on it and the
    # same classify() must report it — nothing here re-implements the rule.
    by_exit = [r for r in inventory if r not in excluded
               and not HUD_TAG.search(_src(r)) and EXIT_REGION.search(_src(r))]
    if not by_exit:
        print("  [FAIL] no game is covered by the inline exit region, so the new "
              "state is untested — that is not a pass", file=sys.stderr)
        failures += 1
    else:
        sample = by_exit[0]
        stripped = EXIT_REGION.sub("<!-- region removed by control -->", _src(sample), count=1)
        really_gone = not EXIT_REGION.search(stripped) and stripped != _src(sample)
        if not really_gone:
            print("  [FAIL] the control did not actually remove the region — a no-op control",
                  file=sys.stderr)
            failures += 1
        else:
            v = classify(sample, has_tag=False, excluded=excluded,
                         has_exit=bool(EXIT_REGION.search(stripped)))
            if v is None:
                print(f"  [FAIL] {sample} with its exit region stripped is still classified as fine",
                      file=sys.stderr)
                failures += 1
            else:
                print(f"  [PASS] control: {sample} with its inline exit region stripped is reported ({v})")
        v_ok = classify(sample, has_tag=False, excluded=excluded, has_exit=True)
        if v_ok is not None:
            print(f"  [FAIL] {sample} WITH its region is reported — the state is always-red",
                  file=sys.stderr)
            failures += 1
        else:
            print(f"  [PASS] negative control: {sample} with its region intact is not a problem")
