...
def spliced_hud_js(data: dict[str, Any], source: str | None = None) -> str:
    ...
    start = source.find(HUD_BEGIN)
    if start < 0:
        raise SystemExit(
            "hud.js: the MBM-HOMEPAGE-CHOICES region is missing. It is not generated into place - "
            "the markers belong to hud.js and this tool only fills between them."
        )
    end = source.find(HUD_END, start)
    if end < 0:
        raise SystemExit("hud.js: unterminated MBM-HOMEPAGE-CHOICES region")
    return source[:start] + hud_homepage_choices(data) + source[end + len(HUD_END):]
