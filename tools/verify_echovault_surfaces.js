...
if (process.env.RF_PUBLICATION === 'games') {
  ...
  published.controls();
  published.verify({ href: HREF, title: 'Echo Vault' }).catch(error => {
  ...
  await gate('C5', 'nothing displaced', () => {
    ...
    assert(slots <= 1, `Echo Vault holds ${slots} slots on the Top Picks rail — a rail slot may only be held once`);
