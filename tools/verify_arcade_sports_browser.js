...
(async()=>{let browser;try{browser=await chromium.launch({headless:true});
 ...
 ok('manifest-count-33',games.length===33,String(games.length));
 ok('manifest-sports-membership',JSON.stringify(sportsManifest)===JSON.stringify(['Apex Kick','Apex Pool','Apex Golf']),JSON.stringify(sportsManifest));
 for(const vp of [{name:'phone',width:390,height:844},{name:'desktop',width:1280,height:900}]){
  const ctx=await browser.newContext({viewport:{width:vp.width,height:vp.height},reducedMotion:'reduce'}),page=await ctx.newPage(),errors=[];
  ...
  await page.waitForFunction(()=>document.querySelectorAll('#sportsRail .gcard').length===3&&document.querySelectorAll('#allGrid .gcard').length===33,{timeout:10000});
  const s=await page.evaluate(()=>{const title=a=>a.querySelector('h3,h4 span')?.textContent.trim()||'';const cards=q=>[...document.querySelectorAll(q)].map(a=>({title:title(a),href:new URL(a.getAttribute('href'),location.href).pathname}));return{sports:cards('#sportsRail .gcard'),top:cards('#topRail .pick'),shelf:cards('#allGrid .gcard'),chips:[...document.querySelectorAll('#chips .chip')].map(x=>x.textContent.trim()),count:document.getElementById('countline').textContent.trim(),scrollW:document.documentElement.scrollWidth,innerW:innerWidth,reduced:matchMedia('(prefers-reduced-motion: reduce)').matches};});
  ...
  ok(vp.name+'-sports-three-adjacent',JSON.stringify(s.sports.map(x=>x.title))===JSON.stringify(['Apex Kick','Apex Pool','Apex Golf']),JSON.stringify(s.sports));
  ok(vp.name+'-sports-links',JSON.stringify(s.sports.map(x=>x.href))===JSON.stringify(['/apexkick/','/apexpool/','/apexgolf/']));
  ok(vp.name+'-apex-kick-remains-top',n(s.top,'Apex Kick')===1,s.top.map(x=>x.title).join(' | '));
  ok(vp.name+'-whole-shelf-complete',s.shelf.length===33&&['Apex Kick','Apex Pool','Apex Golf'].every(t=>n(s.shelf,t)===1),String(s.shelf.length));
  ok(vp.name+'-surface-counts',n(s.top,'Apex Kick')===1&&n(s.sports,'Apex Kick')===1&&n(s.shelf,'Apex Kick')===1&&n(s.sports,'Apex Pool')===1&&n(s.shelf,'Apex Pool')===1&&n(s.sports,'Apex Golf')===1&&n(s.shelf,'Apex Golf')===1);
  ok(vp.name+'-tag-vocabulary',!s.chips.includes('SPORT')&&s.chips.includes('PHYSICS'),s.chips.join(' | '));
  ok(vp.name+'-derived-total',/13 curated favourites of 33 games/.test(s.count),s.count);
  ok(vp.name+'-reduced-no-overflow',s.reduced&&s.scrollW===s.innerW,`${s.scrollW}/${s.innerW}`);
  ok(vp.name+'-zero-errors',errors.length===0,errors.join(' | ')||'none');
