...
(() => {
  ...
  let activeGame=null, returnFocus=null, modalHistory=false;
  ...
  function render({url=true}={}) {
    ...
    for(const card of cards) {
      ...
      card.style.order=s.list==='recent'?String(recent.indexOf(g.id)):'';
    }
  ...
  document.addEventListener('click',e=>{
    ...
    if(link){const id=link.dataset.play;if(byId.has(id)){recent=[id,...recent.filter(x=>x!==id)].slice(0,24);saveList('recent',recent);rememberPosition();}return;}
  ...
  const labels={touch:'Touch controls',keyboard:'Keyboard / mouse',gamepad:'Gamepad',single:'Single-player',local:'Local multiplayer — follow the game’s same-network or shared-device instructions'};
  ...
  function poster(g) {
    ...
    const figure=document.createElement('figure'),img=document.createElement('img'),caption=document.createElement('figcaption');img.src=source;img.alt=m.description||g.title+' cover artwork';img.width=640;img.height=360;caption.textContent=m.poster?'In-game screenshot. '+m.description:'Cover artwork; this is not an in-game screenshot.';figure.append(img,caption);document.getElementById('dialog-media').append(figure);
  }
  function openInfo(g,trigger,watch=false) {
    ...
    if(!dialog.open){dialog.showModal();const u=new URL(location.href);u.hash='game='+encodeURIComponent(g.id);history.pushState({playDialog:true},'',u);modalHistory=true;}
  ...
  function closeInfo(fromHistory=false){if(!dialog.open)return;stopMedia();dialog.close();activeGame=null;if(!fromHistory&&modalHistory){modalHistory=false;history.back();}else modalHistory=false;returnFocus?.focus();}
  ...
  window.addEventListener('popstate',()=>{if(dialog.open&&!history.state?.playDialog)closeInfo(true);applyUrl();render({url:false});});
  ...
  document.querySelectorAll('.game-card img').forEach(img=>img.addEventListener('error',()=>{img.hidden=true;img.closest('figure').querySelector('figcaption').textContent='Image unavailable — game link still works';}));
