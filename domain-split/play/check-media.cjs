...
(async()=>{for(const [engine,type] of [['chromium',chromium],['webkit',webkit]]){
 ...
 for(const width of [390,1280]){
  ...
  for(const g of games){try{
   ...
   const video=page.locator('#game-dialog video');await page.waitForFunction(()=>{const v=document.querySelector('video');return v&&v.currentTime>.4&&!v.paused&&v.readyState>=2;},null,{timeout:10000});
...
})().catch(e=>{report.failures.push({error:e.stack});fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));console.error(e);process.exit(1);});
