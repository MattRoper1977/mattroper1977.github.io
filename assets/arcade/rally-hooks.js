...
// Own at most one frame. A clean menu or held stage sleeps until invalidated.
let as1FrameRequest=0;
function as1ScheduleFrame(){
  if(as1FrameRequest||document.hidden)return;
  if(!as1Dirty&&(game.mode!=='running'||game.paused)&&!v6Director.active&&!v6Ascent.active)return;
  as1FrameRequest=requestAnimationFrame(now=>{as1FrameRequest=0;frame(now);});
}
function as1Invalidate(){as1Dirty=true;as1ScheduleFrame();}
function as1CancelFrame(){if(as1FrameRequest)cancelAnimationFrame(as1FrameRequest);as1FrameRequest=0;}
document.addEventListener('visibilitychange',()=>{if(document.hidden)as1CancelFrame();else as1Invalidate();});
window.addEventListener('pagehide',as1CancelFrame);
window.addEventListener('pageshow',()=>as1Invalidate());
...
function as1SavePreferences(){try{localStorage.setItem(as1PreferenceKey,JSON.stringify(as1Preferences));}catch(_){}as1Invalidate();}
let as1Filter=null;
...
function as1ApplyHold(){
  ...
  v6SyncDrivingState();as1Invalidate();window.MBMArcade?.sync();
}
...
for(const event of ['input','change','click','pointerdown','keydown'])document.addEventListener(event,()=>{as1Invalidate();},{passive:true});
window.addEventListener('resize',()=>{as1Invalidate();},{passive:true});
function as1Bounds(){
  const t=new Track(game.trackDef,'high'),m=t.def.roadWidth,ys=t.samples.map(s=>s.p.y);
...
fixedStep=function(dt){if(game.paused||as1PanelPaused||as1UserPaused)return;const before=game.stageTime;const out=as1FixedStep(dt);if(game.stageTime>before){as1PhysicsCount++;as1SampleClock+=dt;if(as1SampleClock>=.1-1e-8){as1SampleClock-=.1;const p=game.car.pos;if(as1Samples.length<900)as1Samples.push({x:p.x,y:p.y,z:p.z,angle:game.car.heading});}}return out;};
const as1StartStage=startStage;
startStage=function(){as1UserPaused=as1PanelPaused=false;document.documentElement.dataset.as1Held='false';as1Samples=[];as1SampleClock=0;as1Invalidate();return as1StartStage();};
const as1DebugStart=window.RallyVector3D.debugStart;
window.RallyVector3D.debugStart=function(id='alpine'){as1UserPaused=as1PanelPaused=false;document.documentElement.dataset.as1Held='false';as1Samples=[];as1SampleClock=0;as1Invalidate();const out=as1DebugStart(id);window.MBMArcade?.sync();return out;};
let as1IncomingGhost=null,as1IncomingTrack=null;
async function as1LoadGhost(){
  if(!location.hash.startsWith('#ghost='))return;
  try{const samples=window.MBMGhost.decode(location.hash.slice(7),as1Bounds());as1IncomingGhost=samples;as1IncomingTrack=game.trackDef.id;as1InstallGhost();}
...
function as1InstallGhost(){if(!as1IncomingGhost||as1IncomingTrack!==game.trackDef.id)return;if(game.ghostVisual)game.ghostVisual.dispose();game.ghostData=as1IncomingGhost.map((s,i)=>[(i+1)/10,s.x,s.y,s.z,s.angle]);game.ghostVisual=new CarVisual({ghost:true});updateGhostPlayback();as1Invalidate();}
const as1BuildWorld=buildWorld;
buildWorld=function(options){const out=as1BuildWorld(options);as1InstallGhost();as1Invalidate();return out;};
...
window.MBMArcadeHooks={
  ...
  panel:on=>{as1PanelPaused=!!on;if(on&&v6Director.active)v6Director.stop({resume:false,restore:false});as1ApplyHold();},
  setBattery:on=>{as1Battery=!!on;as1Invalidate();},
  ...
  resize:()=>{as1Invalidate();if(renderer)renderer.resize(game.quality==='high'?2:game.quality==='low'?1:1.5);v6SyncRouteCanvas();},
  audioGraph:()=>({ctx:audio.ctx,master:audio.master,filter:as1Filter,output:v6Compressor||audio.master}),
  serialize(){
  ...
  deserialize(data){
    ...
    as1Invalidate();window.MBMArcade?.sync();return true;
  },
