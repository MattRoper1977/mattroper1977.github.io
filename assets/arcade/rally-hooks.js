...
startStage=function(){as1UserPaused=as1PanelPaused=false;document.documentElement.dataset.as1Held='false';as1Samples=[];as1SampleClock=0;as1Dirty=true;return as1StartStage();};
const as1DebugStart=window.RallyVector3D.debugStart;
window.RallyVector3D.debugStart=function(id='alpine'){as1UserPaused=as1PanelPaused=false;document.documentElement.dataset.as1Held='false';as1Samples=[];as1SampleClock=0;as1Dirty=true;const out=as1DebugStart(id);window.MBMArcade?.sync();return out;};
...
function as1InstallGhost(){if(!as1IncomingGhost||as1IncomingTrack!==game.trackDef.id)return;if(game.ghostVisual)game.ghostVisual.dispose();game.ghostData=as1IncomingGhost.map((s,i)=>[(i+1)/10,s.x,s.y,s.z,s.angle]);game.ghostVisual=new CarVisual({ghost:true});updateGhostPlayback();as1Dirty=true;}
