(function(root){'use strict';
  ...
  function open(name){if(!panelName){previousFocus=document.activeElement;appWasInert=hooks.area.inert;hooks.panel(true);hooks.area.inert=true;}panelName=name;panel.hidden=false;body.replaceChildren();status.textContent='';title.textContent={more:'More',comfort:'Comfort',save:'Save code',battery:'Battery'}[name];
    if(name==='more'){for(const key of rows)row(key,{battery:'Battery',comfort:'Comfort',save:'Save code'}[key],()=>open(key));}
    if(name==='battery')toggle('Battery',battery,setBattery);
  ...
  for(const key of rows)control(key,{battery:'Battery',comfort:'Comfort',save:'Save code'}[key],()=>open(key));
  done.onclick=close;
  ...
  async function savePanel(){
    ...
    const capture=row('save','Get save code',async()=>{try{const code=await root.MBMCartridge.encode(hooks.serialize()),thresholds=hooks.thresholds();if(code.length>thresholds.clipboard){download(code,'rally-save.txt');status.textContent='Save file downloaded.';}else{input.value=code;status.textContent=code.length<=thresholds.paper?'Keep this code to bring your progress back.':'Use Copy, or select the code and copy it yourself.';}}catch(e){status.textContent=e.message;}});
  ...
  function layout(){
    ...
    const cs=getComputedStyle(bar),ss=getComputedStyle(shell),buttons=[...bar.children];
    breakpoint=Math.ceil(buttons.reduce((n,e)=>n+e.getBoundingClientRect().width+parseFloat(getComputedStyle(e).marginLeft||0)+parseFloat(getComputedStyle(e).marginRight||0),0)+parseFloat(cs.gap)*(buttons.length-1)+parseFloat(ss.paddingLeft)+parseFloat(ss.paddingRight));
